...
gams_nutriclim_phototrophs_asvs_pc1 <- computeGAMNutriClimAll(pca %>%
...
#   write_csv("output/gams/ela18s_phototrophs_asvs_pc_gams_nutriclim.csv", col_names = TRUE)
...
#ggsave("figures/ela18s_phototrophs_asvs_pc_gams_nutriclim.pdf", gams_nutriclim_phototrophs_asvs_pc_plots, width = 8, height = 7, units = "in", device = cairo_pdf)
...
#                                 var_p_value <= 0.01 & var_p_value > 0.001 ~ "**",
#                                 var_p_value <= 0.001 ~ "***")) %>%
#   write_csv("output/gams/ela18s_phototrophs_asvs_pc_gams_nutriclim_vars.csv", col_names = TRUE)

# Combine contribution plots
(gams_nutriclim_phototrophs_asvs_pc1_contribplot <- plotGAMNutriClimContrib(pca %>%
                                                                              filter(dataset == "Phototroph ASVs"), "PC1"))
#ggsave("figures/ela18s_phototrophs_asvs_pc1_gams_nutriclim_contribplot.pdf", gams_nutriclim_phototrophs_asvs_pc1_contribplot, width = 16, height = 12, units = "in", device = cairo_pdf)
